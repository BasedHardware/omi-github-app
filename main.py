...
async def tool_code_feature(request: Request):
    ...
    try:
        ...
        from claude_coder import generate_code_with_claude, apply_changes_to_repo, create_pr_with_github_api, get_repo_context
        import tempfile
        import git
        import time
        ...
        # Clone repo to get context
        with tempfile.TemporaryDirectory() as tmpdir:
            auth_url = repo_url.replace('https://', f'https://{user["access_token"]}@')
            git.Repo.clone_from(auth_url, tmpdir, depth=1)
            repo_context = get_repo_context(tmpdir)
        ...
        log(f"Applying changes to {repo_full_name}")
        commit_message = f"feat: {feature}\n\nGenerated by Claude AI via Omi"

        result = apply_changes_to_repo(
            repo_url=repo_url,
